...
- (BOOL)matches: (id)item
{
  ...
  //*** Assertion failure in __63-[SenTestingKit_Assertion testAssertsFalseSilentlyWhenExpected]
  //*** Assertion failure in -[SenTestingKit_Assertion testRealAssertionFailuresArePrinted]
  NSString *assertionPrefix = [NSRegularExpression escapedPatternForString:@"*** Assertion failure in "];

  // Build up the regex pattern
  NSMutableString *patternBuilder;
  patternBuilder = [assertionPrefix mutableCopy];
  [patternBuilder appendString:@"(__\\d+)?"];
  [patternBuilder appendString:escapedMethod];
  NSString *pattern = [[patternBuilder copy] autorelease];
  [patternBuilder release];
